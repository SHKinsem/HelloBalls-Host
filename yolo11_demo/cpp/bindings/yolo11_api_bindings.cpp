...
static int g_tensor_order[6] = {0, 1, 2, 3, 4, 5}; // Default order
...
bool ensure_model_initialized() {
    ...
    if (loadModel(g_packed_dnn_handle, g_dnn_handle, g_input_H, g_input_W, g_tensor_order) != 0) {
        py::print("[yolo11_api] Critical: Failed to load model.");
        return false;
    }

    if (hbDNNGetOutputCount(&g_output_count, g_dnn_handle) != 0) {
        py::print("[yolo11_api] Critical: Failed to get model output count.");
    ...
    if (prepareTensors(g_input_tensor, g_output_tensors, g_dnn_handle, g_output_count, g_input_H, g_input_W) != 0) {
        py::print("[yolo11_api] Critical: Failed to prepare model tensors.");
        hbDNNRelease(g_packed_dnn_handle); // Clean up
        return false;
    }

    g_model_initialized = true;
    py::print("[yolo11_api] Model initialized successfully.");
    return true;
}
...
void cleanup_model_resources() {
    if (g_model_initialized) {
        ...
        py::print("[yolo11_api] Model resources released.");
    }
}

// --- PYBIND11 Module Definition ---
PYBIND11_MODULE(yolo11_api, m) {
