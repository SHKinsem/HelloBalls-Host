...
class HelloBallsCV:
    ...
    def handle_keyboard_input(self, key):
        ...
        # Robot control commands
            ...
            if state == ROBOT_STATE_SEARCH:
                self.serial_comm._send_state(self.robot_state, self.search_left_speed, self.search_right_speed, self.tilt_angle)
            else:
                self.serial_comm._send_state(self.robot_state, 0, 0, 0)
    ...
    def process_frame(self):
        ...
        if self.detection_mode == MODE_PERSON_DETECTION:
            detection_results = yolo11_api.inference(preprocessed_frame)
        elif self.detection_mode == MODE_GESTURE_DETECTION:
            detection_results = None
            print("?? Gesture model not loaded yet.")
        else:   
            detection_results = None
            ...
            if person_found:
                ...
                if person is not None:
                    ...
                    if self.is_person_centered(x, w, frame_width):
                        ...
                        offset = person_center_x - frame_center_x 
                        print(f"Offset from center: {offset:.2f}")

                        if offset > 0:
                            print("↪️ Adjusting right to center person...")
                            self.serial_comm._send_state(ROBOT_STATE_SEARCH, +self.person_centering_base_speed, -self.person_centering_base_speed, self.tilt_angle)
                        else:
                ...
                elapsed = time.time() - self.rotation_start_time
                if elapsed < self.max_rotation_duration:
                    self.serial_comm._send_state(ROBOT_STATE_SEARCH, +self.search_turn_speed, -self.search_turn_speed, self.tilt_angle)
                else:
                    self.person_search_rotation_count += 1
                    self.rotation_start_time = time.time()
                    print(f"🔁 Completed search rotation {self.person_search_rotation_count}")
                    ...
                    if self.person_search_rotation_count >= self.max_search_rotations:
                        ...
                        self.serial_comm._send_state(ROBOT_STATE_STOP, 0, 0, self.tilt_angle)
                        self.searching_for_person = False
        ...
        try:
            ...
            if self.serial_comm.connected:
                ...
                for _ in range(3):  # Send multiple stop commands
                    self.serial_comm._send_state(ROBOT_STATE_STOP, 0, 0, 0)
                    time.sleep(0.1)
    ...
    def control_robot(self):
        ...
        if self.robot_state == ROBOT_STATE_CHASE_BALL:
            ...
            if USE_MCU_CHASE:
                self.serial_comm.send_command(ROBOT_STATE_CHASE_BALL, 0, 0, 0, 0)
                return
            ...
            if self.best_target is not None:
                    ...
                    if distance != self.last_distance_bin:
                        ...
                        if self.distance_stable_start_time is not None:
                            ...
                            if stable_duration >= 2.0 and not self.serve_triggered:
                                ...
                                self.serial_comm._send_state(ROBOT_STATE_DELIVER_BALL, 0, speed, angle)
                ...
                if distance is None or self.serve_triggered:
                    ...
                    self.serial_comm._send_state(ROBOT_STATE_DELIVER_BALL, 0, 0, self.tilt_angle)
            else:
        ...
        if self.robot_state == ROBOT_STATE_SEARCH:
            self.serial_comm._send_state(self.robot_state, left_speed, right_speed, self.tilt_angle)
        elif self.robot_state == ROBOT_STATE_DELIVER_BALL:
            self.serial_comm._send_state(self.robot_state, left_speed, right_speed, self.tilt_angle)
        else:
            self.serial_comm._send_state(self.robot_state, left_speed, right_speed, 0)
