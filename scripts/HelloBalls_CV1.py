...
ROBOT_STATE_INIT = 5  # Initial state when robot is starting up --> rotate to find person
ROBOT_STATE_NAMES = ["STOP", "CHASE_BALL", "RETURN_HOME", "DELIVER_BALL", "SEARCH","INIT"]
    ...
    def process_frame(self):
        ...
        if self.detection_mode == MODE_PERSON_DETECTION and self.searching_for_person:
            ...
            if person_found:
                ...
                self.person_search_rotation_count = 0
                self.rotation_start_time = None
                print("✅ Person found! Stopping init mode.")
                ...
                while self.person_search_rotation_count < 3:
                    self.serial_comm.send_command(
                            2, 
                            +100,  
                            -100, 
                    ...
                    # self.rotation_start_time = time.time()  # record time for every cycle

                    # while time.time() - self.rotation_start_time < self.max_rotation_duration:
                    #     self.serial_comm.send_command(
                    #         ROBOT_STATE_INIT, 
                    #         +100,  
                    #         -100, 
