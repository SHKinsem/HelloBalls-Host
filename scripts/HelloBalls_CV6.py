...
sys.path.append("/home/sunrise/.local/lib/python3.10/site-packages/bpu_infer_lib")
from bpu_infer import BPUModel
...
try:
    ...
    print("Successfully imported yolo11_api module",yolo11_api.__file__)
...
'''
gesture_model=None
gesture_model_path="/home/sunrise/Documents/HelloBalls-Host/yolo11_gesture/cpp/try1/converted_model.bin"

# === YOLO gesture labels: 按 conf.yaml 的 names 顺序改 ===
GESTURE_NAMES = ["one", "call", "ok", "fist", "palm"] # TODO: 用你的 conf.yaml 顺序

# === 预处理 & 后处理工具 ===
def _letterbox(bgr, new_shape=(640, 640), color=(114,114,114)):
    h0, w0 = bgr.shape[:2]
    ...
    img = cv2.resize(bgr, new_unpad, interpolation=cv2.INTER_LINEAR)
...
def _bgr_to_nv12(bgr):
    ...
    y_size = w*h
    uv_size = (w//2)*(h//2)
    y = yuv_i420[0:y_size].reshape(h, w)
    u = yuv_i420[y_size:y_size+uv_size].reshape(h//2, w//2)
    v = yuv_i420[y_size+uv_size:y_size+2*uv_size].reshape(h//2, w//2)
    uv = np.empty((h//2, w), dtype=np.uint8)
    uv[:, 0::2] = u
    uv[:, 1::2] = v
    return np.vstack([y, uv])

def _nms(boxes, scores, iou_thres=0.45, topk=300):
...
MODE_GESTURE_DETECTION = 2  # modify: Placeholder for future gesture detection
MODE_NAMES = ["Ball Detection", "Person Detection", "Gesture Detection"]
...
class HelloBallsCV:
    ...
    def __init__(self, show_preview=True, detection_mode=MODE_PERSON_DETECTION, serial_port='/dev/ttyS1'):
        ...
        self.model_mode = "gesture"    # 只测手势，先跑通
        self.gesture_input = os.environ.get("GESTURE_INPUT", "BGR").upper()  # 'BGR' 或 'NV12'
        ...
        #modifty: look at person and regcognize gesture
        # 状态控制
        self.state = 'searching'  # 'searching' | 'found_person' | 'raising_head' | 'gesture_mode' | 'lost_person' | 'idle'

        # 距离和角度参数
        self.detected_distance = None
        self.has_raised_head = False
        self.gesture_mode_counter = 0  # 用于定期fallback回找人模型

        self.gesture_mode_resolution_toggled = False
    ...
    def initialize(self):
        ...
        try:
            if self.model_mode == "person":
                ...
                # Load gesture YOLO .bin via BPUModel
                try:
                    
                    model_abspath = os.path.abspath(gesture_model_path)
                    if not os.path.exists(model_abspath):
                        print(f"Error: gesture model not found: {model_abspath}")
                        return False
                    gesture_model = BPUModel(model_abspath)
                    self.model_initialized = True
                    print(f"Gesture YOLO model loaded: {model_abspath}")
                except Exception as e:
                    print(f"Error initializing gesture model: {e}")
                    return False

            elif self.model_mode == "gesture":
                # Placeholder for gesture model initialization
                # self.model_initialized = gesture_api.initialize_model()
                self.model_initialized = True
                # Load gesture YOLO .bin via BPUModel
                try:
    ...
    def toggle_resolution_short(self):
        """Toggle camera resolution
    ...
    def toggle_resolution_long(self):
        if not self.camera or not self.camera.isOpened():
            return 
    ...
    def process_frame(self):
        ...
        # 2) 预处理：用你现有的 letterbox，把映射参数拿出来用于回到原图坐标
        preprocessed_frame, x_scale, y_scale, x_shift, y_shift = self.preprocess_image_letterbox(frame)
        # 注意：你的 preprocess 已经把图像变成模型尺寸(通常 640x640)且加了 pad，
        # 后面你现有的坐标反变换正是用这些参数。

        # 3) —— 只跑“手势(=检测)”模型 —— 
        # 为了快速测试：不管 self.model_mode 是 "person" 还是 "gesture"，都跑手势模型
        ...
        try:
            # (a) 准备输入：先用 BGR 最省事；若你的 .bin 需要 NV12，把下一行改为：inp = bgr_to_nv12(preprocessed_frame)
            inp = preprocessed_frame

            # (b) 前向
            raw = gesture_model.forward(inp)   # 你在 initialize() 里用 BPUModel 加载的全局模型

            # (c) 解析 YOLO 输出：形状常见为 (N, 5 + num_classes)
            out = np.array(raw)
            num_classes = len(GESTURE_NAMES)

            if out.ndim == 1:
                pred = out.reshape(-1, 5 + num_classes)
            elif out.ndim == 2 and out.shape[1] >= 5:
                pred = out
            else:
                pred = out.reshape(-1, 5 + num_classes)

            boxes_xywh = pred[:, :4]              # [cx, cy, w, h] —— 在“预处理后图像(640域)”坐标系
            obj = pred[:, 4]                      # objectness
            cls_scores = pred[:, 5:]              # per-class scores

            cls_ids = np.argmax(cls_scores, axis=1)
            cls_conf = cls_scores[np.arange(len(cls_scores)), cls_ids]
            scores = obj * cls_conf               # 置信度 = obj * class_conf
            ...
            if np.any(mask):
                ...
                # (d) xywh -> x1y1x2y2（仍然在“预处理后图像”坐标系）
                xy = boxes_xywh[:, :2]
                wh = boxes_xywh[:, 2:]
                x1y1 = xy - wh / 2
                x2y2 = xy + wh / 2
                boxes_x1y1x2y2 = np.hstack([x1y1, x2y2])

                # (e) NMS（在预处理坐标系做）
                keep = _nms(boxes_x1y1x2y2, scores, iou_thres=0.45, topk=300)
                boxes_x1y1x2y2 = boxes_x1y1x2y2[keep]
                scores = scores[keep]
                cls_ids = cls_ids[keep]

                # (f) 按你“原有的”反变换方式回到原图坐标：
                # 你的后续代码期望 detection_results.bboxes 是“预处理坐标系”的 x1,y1,w,h，
                # 然后它会用 (x - x_shift)/x_scale 把坐标变回原图，所以这里给它 x1,y1,w,h 即可。
                class SimpleDet: pass
                detection_results = SimpleDet()
                detection_results.class_ids = []
                detection_results.bboxes = []
                detection_results.scores = []

                for (x1, y1, x2, y2), s, c in zip(boxes_x1y1x2y2, scores, cls_ids):
                    w_box = x2 - x1
                    h_box = y2 - y1
                    detection_results.class_ids.append(int(c))
                    detection_results.bboxes.append([float(x1), float(y1), float(w_box), float(h_box)])  # 预处理坐标
                    detection_results.scores.append(float(s))

                    # 你屏幕上想直接画框的话，也可以先画“原图坐标”的框：
                    # ox = (x1 - x_shift) / x_scale; oy = (y1 - y_shift) / y_scale
                    # ow = w_box / x_scale;        oh = h_box / y_scale
                    # cv2.rectangle(frame, (int(ox), int(oy)), (int(ox+ow), int(oy+oh)), (0,255,0), 2)
                    # cv2.putText(frame, f"{GESTURE_NAMES[int(c)]} {s:.2f}", (int(ox), max(0,int(oy)-6)),
            ...
            print(f"[Gesture] inference/parse failed: {e}")
            detection_results = None

        # 4) —— 下面保留你“原来的处理逻辑”不动（它会把框转换回原图坐标，并更新 self.detected_objects/self.best_target）
        # Reset detection results
        self.detected_objects = []
        self.best_target = None
        closest_to_center_distance = float('inf')

        # Process detection results（沿用你原来的这段）
        if detection_results and len(detection_results.class_ids) > 0:
            for cls_id, boxes, confs in zip(detection_results.class_ids,
                ...
                # Convert bbox from "preprocessed coords" back to ORIGINAL frame coords（用你原来的公式）
        ...
        # ====== 你后面的状态机 / 串口 / UI 保持不动 ======
        # （注意：如果你现在只想测手势，建议把 self.state 固定成 'gesture_mode_1'，避免走到找人逻辑）
        print("person_grature_loop_state =", self.state)

        # ... 这里起保留你原来的状态机分支 searching / raising_head / gesture_mode_1 / lost_person ...
    ...
    def process_frame(self):
        ...
        if self.model_mode == "person":
            ...
            #detection_results = gesture_api.inference(preprocessed_frame)  # 替换成你的手势模型

        elif self.model_mode == "gesture":
            print("gesture recognization mode, not ready")
            detection_results = None
            #detection_results = yolo11_api.inference(preprocessed_frame)
            #detection_results = gesture_api.inference(preprocessed_frame)  # 替换成你的手势模型
        else:
        ...
        if self.state == 'searching':
            ...
            if person_found:
                # 找到人后选出最佳目标（默认第一个）
                self.making_player_at_center()
                if self.is_person_centered():  # 你已有检测函数
                    self.detected_distance = self.estimate_distance_from_bbox(x, y, w, h, confs)# 距离估计函数（你已有）
                    print("detected_distance = ", self.detected_distance)
                    if self.detected_distance in [None,3,6]:
                        self.state = 'raising_head'
                        self.gesture_mode_resolution_toggled = True
                        self.toggle_resolution_long()
                    else:
                        self.gesture_mode_resolution_toggled = True
                        self.toggle_resolution_short()
            else:
                # 没找到人，原地旋转找人
                self.rotate_to_find_player()

        elif self.state == 'raising_head':
            self.serial_comm.send_command(ROBOT_STATE_SEARCH, 0, 0, 35)# 抬头
            self.has_raised_head = True
            self.state = 'gesture_mode_1'
            self.gesture_mode_counter = 0

            
        elif self.state == 'gesture_mode_1':
            # 模拟手势识别占位
            #print("[gesture not ready, skip...]")
            self.model_mode = "gesture"
            if not self.gesture_mode_resolution_toggled:
                print("switch camera")
                self.gesture_mode_resolution_toggled = True
            # 模拟周期性fallback：每5帧切回找人模型
            self.gesture_mode_counter += 1
            if self.gesture_mode_counter % 15 == 0:
                self.model_mode = "person"
                person_found = any(obj['class_id'] == PERSON_CLASS for obj in self.detected_objects)
                if not person_found:
                    self.toggle_resolution_short()
                    self.state = 'lost_person'
        ...
        elif self.state == 'lost_person':
            self.rotate_to_find_player35()  # 35degree找人
            person_found = any(obj['class_id'] == PERSON_CLASS for obj in self.detected_objects)
            if person_found:
                self.making_player_at_center()
                if self.is_person_centered():
                    self.state = 'gesture_mode_1'

            else:
                self.serial_comm.send_command(ROBOT_STATE_SEARCH,0,0,0)
    ...
    def is_person_centered(self, tolerance=0.1):
        """
    ...
    def making_player_at_center(self):
        person = next((obj for obj in self.detected_objects if obj['class_id'] == PERSON_CLASS), None)
    ...
    def rotate_to_find_player(self):
        # 没找到人，原地旋转找人
        ...
        if elapsed < self.max_rotation_duration:
            self.serial_comm.send_command(ROBOT_STATE_SEARCH, +self.search_turn_speed, -self.search_turn_speed,0)
        else:
    ...
    def rotate_to_find_player35(self):
        # 没找到人，原地旋转找人
        if self.rotation_start_time is None:
            self.rotation_start_time = time.time()
            print("🔄 Starting person search rotation...")
        ...
        elapsed = time.time() - self.rotation_start_time
        if elapsed < self.max_rotation_duration:
            self.serial_comm.send_command(ROBOT_STATE_SEARCH, +self.search_turn_speed, -self.search_turn_speed, 35)
        else:
            self.person_search_rotation_count += 1
            self.rotation_start_time = time.time()
            print(f"🔁 Completed search rotation {self.person_search_rotation_count}")
